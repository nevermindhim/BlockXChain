...
func TestnetCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a BlockX testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).

Note, strict routability for addresses is turned off in the config file.`,

		Example: "blockxd testnet --v 4 --keyring-backend test --output-dir ./output --starting-ip-address 192.168.10.2",
	...
	cmd.Flags().String(flagNodeDaemonHome, "blockxd", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "blockxcli", "Home directory of the node's cli configuration")
	...
	cmd.Flags().String(flagCoinDenom, ethermint.AttoBlockX, "Coin denomination used for staking, governance, mint, crisis and evm parameters")
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", ethermint.AttoBlockX), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01abcx,0.001stake)")
