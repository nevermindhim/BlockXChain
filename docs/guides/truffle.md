...
Set up a Truffle BlockX local development environment. {synopsis}
...
[Truffle](https://www.trufflesuite.com/truffle) is a development framework for deploying and managing [Solidity](https://github.com/ethereum/solidity) smart contracts. In this guide, we will learn how to deploy a contract to a running BlockX network.
...
You will also need to install BlockX. Check this [document](./../quickstart/installation.md) for the full instructions.
...
This will allow your contract to connect to your BlockX local node.
...
```bash
blockxcli rest-server --laddr "tcp://localhost:8545" --unlock-key mykey--chain-id 8 --trace
```
...
You should see incoming deployment logs in the BlockX daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the oether to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the BlockX node using the `test` command:
...
Learn how to connect BlockX to [Metamask](./../guides/metamask.md) {hide}
