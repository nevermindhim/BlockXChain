...
var _ Backend = (*BlockXBackend)(nil)

// BlockXBackend implements the Backend interface
type BlockXBackend struct {
	ctx       context.Context
...
// New creates a new BlockXBackend instance
func New(clientCtx clientcontext.CLIContext) *BlockXBackend {
	return &BlockXBackend{
...
func (b *BlockXBackend) BlockNumber() (hexutil.Uint64, error) {
...
func (b *BlockXBackend) GetBlockByNumber(blockNum rpctypes.BlockNumber, fullTx bool) (map[string]interface{}, error) {
...
func (b *BlockXBackend) GetBlockByHash(hash common.Hash, fullTx bool) (map[string]interface{}, error) {
...
func (b *BlockXBackend) HeaderByNumber(blockNum rpctypes.BlockNumber) (*ethtypes.Header, error) {
	height := blockNum.Int64()
	if height <= 0 {
		// get latest block height
		num, err := b.BlockNumber()
...
func (b *BlockXBackend) HeaderByHash(blockHash common.Hash) (*ethtypes.Header, error) {
...
func (b *BlockXBackend) GetTransactionLogs(txHash common.Hash) ([]*ethtypes.Log, error) {
...
func (b *BlockXBackend) PendingTransactions() ([]*rpctypes.Transaction, error) {
	...
	for _, tx := range pendingTxs.Txs {
		...
		if err != nil {
			// ignore non BlockX EVM transactions
			continue
		}
...
func (b *BlockXBackend) GetLogs(blockHash common.Hash) ([][]*ethtypes.Log, error) {
	res, _, err := b.clientCtx.Query(fmt.Sprintf("custom/%s/%s/%s", evmtypes.ModuleName, evmtypes.QueryHashToHeight, blockHash.Hex()))
	if err != nil {
		return nil, err
	}
...
func (b *BlockXBackend) BloomStatus() (uint64, uint64) {
	return 4096, 0
}

// LatestBlockNumber gets the latest block height in int64 format.
func (b *BlockXBackend) LatestBlockNumber() (int64, error) {
